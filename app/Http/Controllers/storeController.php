...
class storeController extends Controller
{
    ...
    public function koleksi()
    {
        $iduser = session()->get('id_anggota');
        if (request('search_hal')) {
            $data['buku'] = DB::table('tampil_koleksi')->where('id_user', $iduser)->where('judul', 'like', '%' . request('search_hal') . '%')->get();
        } else {
            $data['buku'] = DB::table('tampil_koleksi')->where('id_user', $iduser)->get();
        }
        $data['title'] = "Home";
    ...
    {
        $iduser = session()->get('id_anggota');
        $data['detail'] = DB::table('tampil_koleksi')->where('id_buku', $id)->where('id_user', $iduser)->first();
        // dd($data);
        $data['title'] = "Home";
    ...
    public function tmbh_koleksi(Request $request, string $id)
    {
        $iduser = session()->get('id_anggota');
        $query = DB::table('buku')->where('id', $id)->first();
        $koleksi = DB::table('koleksi')->where('id_buku', $query->id)->where('id_user', $iduser)->first();
        if ($koleksi != null) {
            return redirect('/hal_utama')->with('warning', 'Buku Sudah Ada !');
        } else {
            $koleksi = new koleksi();
    ...
    public function riwayat()
    {
        ...
        $iduser = session()->get('id_anggota');
        $data['riwayat'] = DB::table('tampil_riwayat')->where('id_user', $iduser)->get();
        $data['title'] = "Home";
    ...
    {
        $data = DB::table('tampil_riwayat')->where('id', $id)->first();
        $ulasan = new ulasan();
    ...
    {
        $data['ulasan'] = DB::table('tampil_ulasan')->where('id_buku', $id)->get();
